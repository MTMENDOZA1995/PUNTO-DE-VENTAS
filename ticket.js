...
// ==========================================
function imprimirTicketGlobal(ventaObj) {
    if(!ventaObj) return alert("Error al encontrar los datos de la venta para imprimir.");

    // 1. OBTENER DATOS DE LA EMPRESA EN TIEMPO REAL DESDE "CONFIGURACIÓN"
    const configGuardada = JSON.parse(localStorage.getItem('isa_config')) || {};
    
    // Si no hay configuración guardada aún, usamos unos datos por defecto
    const DATOS_EMPRESA = {
        nombre: configGuardada.empresa || "INNOVACIÓN SOSTENIBLE & ATRACTIVA",
        ruc: configGuardada.ruc || "10733720323",
        direccion: configGuardada.direccion || "Jr. Belaunde Terry C-10 Mollepampa Alta",
        celular: configGuardada.telefono || "952 580 740",
    ...
    if(ventaObj.productos && ventaObj.productos.length > 0) {
        ...
        // Soporte para ventas antiguas que solo tenían un producto
        productosHTML += `
    ...
    // 3. LÓGICA INTELIGENTE DE FILAS (Descuento e Impuestos)
    ...
    // 4. DISEÑO DEL TICKET INYECTANDO VARIABLES
    const ticketHTML = `
        <!DOCTYPE html>
        <html>
        <head>
    ...
    // 5. APERTURA Y EJECUCIÓN DE IMPRESIÓN
    const printWindow = window.open('', '_blank', 'width=400,height=600');
    printWindow.document.write(ticketHTML);
    printWindow.document.close();
    printWindow.focus();
    
    // Le damos 500ms al navegador para cargar la fuente del código de barras antes de imprimir
    setTimeout(() => {
        printWindow.print();
        printWindow.close();
    }, 500);
